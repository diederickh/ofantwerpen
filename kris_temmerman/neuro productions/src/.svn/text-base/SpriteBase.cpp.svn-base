...
void SpriteBase::setup(){

    GLfloat w = 190.0 +((float)rand()/RAND_MAX)*200;
    GLfloat h =50.0+((float)rand()/RAND_MAX)*80;
...
void SpriteBase::update()
{
    ...
    y+=(targetY-y)/100;
   if((float)rand()/RAND_MAX > 0.9999) targetY = ((float)rand()/RAND_MAX-0.5)*500;
    if((float)rand()/RAND_MAX > 0.999) targetZ =-((float)rand()/RAND_MAX) *500 -400;
    float tempZ =z;
...
void SpriteBase::setZ(float zPos)
{
